    ...
    config.to_prepare do
      ...
      unless Redmine::Plugin.registered_plugins.include?(:openproject_release)
        Redmine::Plugin.register :openproject_release do
          ...
          OpenProject::Footer.add_content('release_version', proc { I18n.t('op_release_current_version', version: OpenProject::Release.version) if User.current.admin? })
          OpenProject::Footer.add_content('release_date', proc { I18n.t('op_release_last_update', released_at: OpenProject::Release.release_date) })
        end
      end
    end
  end
